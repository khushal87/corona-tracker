...
import './CasesConfirmed.css';

class CasesConfirmed extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: [],
    ...
    render() {
        return (
            <div className="countries-data">
                <h3 className="heading">Cases <span style={{ color: "red" }}> Confirmed </span> by Country/Region</h3>
                ...
                <input name="search" className="search-input" onChange={this.onChangeHandler} />
                {
                    this.state.filter_data.map((indiData, key) => {
                        return <div className="country-data" key={key}>
                            <div className="country-name">{indiData.country_name + " "} <div className="country-cases">{indiData.cases}</div></div>
                        </div>
                    })
                }
            </div>
